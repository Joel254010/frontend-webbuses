...
function PainelAdmin() {
  ...
  const carregarAnuncios = async () => {
    try {
      ...
      // ✅ compat: novo formato { data, paginaAtual, ... }
  ...
  const excluirAnuncio = async (anuncioId) => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        alert("❌ Erro ao excluir anúncio: " + (e?.mensagem || "Erro desconhecido."));
      }
    } catch (erro) {
      ...
      {anunciantes.map((anunciante) => (
        <div key={anunciante.id} style={styles.cardAnunciante}>
          ...
          {anunciante.anuncios.map((anuncio) => {
            const capa = `${API_URL}/anuncios/${anuncio._id}/capa`; // ✅ capa leve
            const fotosTotal = anuncio.imagensCount ?? 0;          // ✅ contador vindo do backend
            return (
              <div key={anuncio._id} style={styles.cardAnuncio}>
                <div style={styles.galeria}>
                  <img src={capa} alt="Capa" style={styles.imagemMiniatura} loading="lazy" />
                </div>
