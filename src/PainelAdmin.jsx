...
function PainelAdmin() {
  ...
  const carregarPagina = useCallback(
    ...
    [limit, agrupar] // ✅ somente dependências reais
  );
  ...
  const toggleFotos = async (anuncio) => {
    ...
    if (!fotosMap[id]) {
      try {
        ...
        let fotos = Array.isArray(full.imagens)
          ? full.imagens.filter(Boolean)
          : [];
  ...
  const carregarLeads = useCallback(
    ...
    [leadsLimit] // ✅ somente dependências reais
  );
  ...
  const leadsFiltrados = useMemo(() => {
    ...
    if (interesseLead)
      base = base.filter((l) => (l.interesse || "") === interesseLead);
    return base;
                      ...
                      <div style={styles.itemInfo}>
                        ...
                        anuncio.status === "aguardando pagamento" ? (
                          ...
                          <div style={styles.actions}>
                            ...
                            <button
                              onClick={() =>
                                atualizarStatusAnuncio(id, "vendido")
                              }
                              style={styles.btnWarn}
                            >
                              Confirmar venda
      ...
      <section style={styles.card}>
        <div style={styles.cardHeader}>
          ...
          <div style={styles.leadActions}>
            ...
            <button onClick={baixarCSV} style={styles.btnOk}>
              Exportar CSV
            </button>
          </div>
        </div>
        ...
        <div style={styles.pager}>
          ...
          <button
            onClick={() =>
              carregarLeads(
                Math.min(totalPaginasLeads, leadsPage + 1),
                leadsLimit
              )
            }
